...
st.header('Train')
st.write(train['GB'].value_counts())
st.header('Test')
st.write(test['GB'].value_counts())

train_df = train.copy()
test_df = test.copy()

a = Resampling(dataset=train_df, target_col='GB', bads_label='BAD',
               goods_label='GOOD')  # initialize our ensemble learning
######################################
#     There was ratio_start=0.15     #
######################################
a.balanced_resamples_ratio(ratio_start=0.15, ratio_end=0.20)  # balance by ratio
...
a.ensemble_train(algorithm='gradb')
